...
const RollingSharpeChart = ({ data, syncId }: Props) => {
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const window = data.slice(i - WINDOW + 1, i + 1).map(d => d.logReturn).filter((v): v is number => v !== null);
    ...
    <div className="chart-container">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <AreaChart data={chartData} syncId={syncId}>
          ...
          <Area type="monotone" dataKey="sharpe" stroke={UP_COLOR} fill="url(#sharpeFillPos)" strokeWidth={1.5} dot={false} connectNulls={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
